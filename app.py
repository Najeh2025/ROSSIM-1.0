...
# ── ONGLET 1 : CAMPBELL ───────────────────────────────────────────────────
    ...
    with tab_stab:
        ...
        with col_c:
            cxy_val = st.slider("Amortissement croisé Cxy (N.s/m)", 0, int(1e4), 0, step=int(100), key="m3_cxy")
...
def _call_ross_gpt(user_msg: str, context: dict, history: list) -> str:
    ...
    try:
        ...
        system_prompt = f"""Tu es SmartRotor Copilot, un ingénieur expert en dynamique des rotors, mécanique vibratoire et spécialiste absolu de la bibliothèque Python ROSS.
